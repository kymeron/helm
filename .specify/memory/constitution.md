...
- 修订章节 (v1.2.0):
  - 将『Vercel KV 云端快照同步』纳入 v1.2+ In Scope，作为 Vercel 部署场景下的可选增强
  - Out of Scope 中移除『服务器侧持久化』与『公开互联网同步』，改为「除 Vercel KV 快照同步外的服务器侧持久化/公开互联网同步」
  - 原则 VI 示例更新，区分 LAN 同步与 Vercel 云端同步两种可选增量
- 删除章节: 无
- 受影响模板:
...
### VI. 设备与网络边界清晰 (v1.1 新增, v1.2.0 修订)

HELM MUST 区分**核心本地能力**与**可选增量**;两者解耦、可独立关闭。

- **核心层** (MUST,离线可用):本地 CRUD、看板、统计、筛选、JSON 导入导出。不得依赖任何运行时网络资源。
- **可选增量** (MAY,默认关闭时可降级):
  - 局域网同步(iPad 与手机等设备在同 WiFi 下的数据互通)。
  - Vercel KV 云端快照同步(Vercel 部署场景下 PC / 手机跨设备同步,依赖 Vercel KV + Serverless Functions)。
  - 移动端响应式布局、软删除墓碑等。
- 增量 MUST 不修改核心 API 契约;仅以新增 action / hook / 组件 / API 路由的形式接入。
- 增量失败 MUST 不得影响核心层可用性(同步断连应可降级为纯本地模式)。

**理由**: 个人场景从『一台电脑』演进到『多设备同 WiFi』再到『Vercel 部署跨设备同步』是自然升级路径。把增量作为可选层接入,既保留宪法的『本地优先』契约,又允许能力按需启用而不产生技术债。
...
- Vercel KV 云端快照同步(v1.2+ 可选增量,用于 Vercel 部署后 PC / 手机跨设备数据一致)。

**v1 范围外(Out of Scope)**:
- 多用户协作、鉴权 / 登录。
- 推送通知、定时提醒、子任务、任务依赖。
- 除 Vercel KV 快照同步外的服务器侧持久化与公开互联网同步。
- 浅色主题切换(留口子但 v1 不实现)。
...
**版本**: 1.2.0 | **批准日期**: 2026-06-19 | **最后修订**: 2026-06-21