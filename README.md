...
> 面向个人使用的驾驶舱看板:记录想法、跟踪进度、用数据辅助决策。PC / iPad / 手机均可使用,同 WiFi 下数据互通。
...
[![Sync](https://img.shields.io/badge/Sync-LAN_WebSocket-teal)](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket)
...
│   ├── hooks/              # 自定义 Hooks:useTasks / useStats / useSync
│   ├── lib/                # 纯函数:db / filters / stats / state-machine / time / export / sync
...
│   └── main.tsx
├── tests/
│   └── unit/               # 单元测试(filters / state-machine / stats / sync / time)
...
HELM 严格遵循 `.specify/memory/constitution.md`(v1.1.0)中定义的核心原则:
...
6. **设备与网络边界清晰** — 核心本地能力 MUST 离线可用;LAN 同步与移动端响应式作为可选增量接入,失败不影响核心层
