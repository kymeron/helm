...
> 面向个人使用的驾驶舱看板:记录想法、跟踪进度、用数据辅助决策。PC / iPad / 手机均可使用,同 WiFi 下数据互通;部署到 Vercel 后,跨设备通过 Upstash Redis 云端同步。
...
[![Cloud](https://img.shields.io/badge/Cloud-Upstash_Redis-00E9A3)](https://upstash.com/docs/redis)
[![Sync](https://img.shields.io/badge/Sync-LAN_WebSocket_+_Upstash-teal)](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket)
...
- **Upstash 云端同步(可选)**:部署到 Vercel 后,通过 Upstash Redis 在 PC / 手机间同步完整任务快照;无长连接、离线可继续编辑
...
| 云端同步 | Upstash Redis + Serverless Functions | Vercel 部署跨设备快照同步 |
...
- **服务端**:`api/sync.ts` 是 Vercel Serverless Function,通过 Upstash Redis REST API 读写任务快照(零 Vercel 集成依赖,无 `@vercel/kv`)
...
- **离线可用**:网络失败时降级为本地模式,恢复后自动重试;未配置 Upstash 时 `/api/sync` 返回 503,客户端保持 `idle`

### 部署配置

1. 注册 [Upstash Console](https://console.upstash.com/),**Create Database**,选 region(推荐 `ap-southeast-1` 新加坡 / `hkg1` 香港),开启 TLS。
2. 在数据库详情页复制 `REST URL` 和 `REST Token`。
3. 回到 Vercel → `helm-weld-three` → **Settings → Environment Variables**,新增:
   - `UPSTASH_REDIS_REST_URL` = `https://xxx.upstash.io`
   - `UPSTASH_REDIS_REST_TOKEN` = `AXxx...`
   - 三个环境(Production / Preview / Development)都勾上。
4. 在 Vercel **Deployments** 页面 Redeploy 最新一次部署。
5. 部署完成访问 `https://helm-weld-three.vercel.app/api/sync?token=test`,应返回 `{"tasks":[],"updatedAt":null}`。
6. PC 端打开部署地址,复制带 `?token=xxx` 的 URL 到手机浏览器,即可看到同一份数据。
...
| Upstash 免费层 | 256 MB / 10k 命令/天,个人使用足够 |
...
6. **设备与网络边界清晰** — 核心本地能力 MUST 离线可用;LAN 同步、Upstash 云端同步与移动端响应式作为可选增量接入,失败不影响核心层
...
| ✅ M8 | Upstash Redis 云端同步(跨设备数据一致) | 已完成 |
