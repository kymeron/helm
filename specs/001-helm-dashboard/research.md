...
本文件记录 Phase 0 研究结论，解决 Technical Context 中的所有技术选型与最佳实践问题。所有决策已与 `.specify/memory/constitution.md` v1.0.0 对齐。
...
**Rationale**:
- HELM 状态规模小（任务列表 + UI 偏好），Zustand 的 store+selector 模型足够。
- 无 boilerplate（对比 Redux Toolkit 的 configureStore/createSlice/reducer 仪式感）。
- 天然支持订阅外部变更（IndexedDB → store），便于本地优先架构。
...
- UI 偏好（筛选/排序/时间范围）数据量极小且需同步读取，localStorage 足矣。
...
**Decision**: ECharts（经 echarts-for-react）

**Rationale**:
- spec 要求环形图（类型分布）+ 折线图（完成趋势），ECharts 两者均原生支持且视觉精致。
...
**Rationale**:
- tasksSlice 持有任务全量；`useStats(tasks, timeRange)` 用 useMemo 派生指标与图表数据。
...
**覆盖分层**:
1. **纯逻辑（必测，TDD）**: state-machine / stats / filters / time —— 单元测试，覆盖所有分支与边界。
...
**Rationale**:
- 结构带版本号便于未来导入兼容。
- `tasks` 为完整 Task 数组，与 IndexedDB 存储结构一致，导入即全量替换。
...
**初始化命令序列**（供 tasks.md 引用）:
```bash
npm create vite@latest . -- --template react-ts
npm install zustand @dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities echarts echarts-for-react idb lucide-react
...
所有 Technical Context 项已明确，无 `NEEDS CLARIFICATION` 残留。技术选型与宪法 v1.0.0 五原则一致，可进入 Phase 1 设计数据模型与契约。
